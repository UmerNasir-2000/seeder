    ...
    try {
      ...
      if (data.data.movie.torrents !== undefined) {
        for (const torrent of data.data.movie.torrents) {
          const moviesTorrent = await connection.execute(
            `INSERT INTO movie_torrents(movie_id, hash, quality, type, seeds, peers, size_bytes) VALUES(${rows.insertId},"${torrent.hash}","${torrent.quality}","${torrent.type}",${torrent.seeds}, ${torrent.peers}, ${torrent.size_bytes})`
          );
        }
      }
      ...
      if (data.data.movie.cast !== undefined) {
        for (const artist of data?.data?.movie?.cast) {
          if (!artistsObj[artist.imdb_code]) {
            let imageURL = artist.url_small_image
              ? artist.url_small_image
              : null;

            if (imageURL == null) {
              const [result, field] = await connection.execute(
                `INSERT INTO artists(full_name, imdb_code) VALUES("${artist.name}", "${artist.imdb_code}")`
              );

              if (result.insertId !== 0)
                artistsObj[artist.imdb_code] = result.insertId;
            } else {
              const [result, field] = await connection.execute(
                `INSERT INTO artists(full_name, image_url, imdb_code) VALUES("${artist.name}", "${imageURL}", "${artist.imdb_code}")`
              );
          ...
          let res = checkDoubleQuote(artist.character_name);
          const characterName = res ? `'${artist.character_name}'` : `"${artist.character_name}"`

          await connection.execute(
            `INSERT INTO movie_cast_mapping(movie_id, artist_id, character_name) VALUES(${
              rows.insertId
            }, ${artistsObj[artist.imdb_code]}, ${characterName})`
          );
        }
      }
...
function checkDoubleQuote(name) { 
  const res = name.indexOf('"') >= 0
  return res
}
